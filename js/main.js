...
----------------------------------------------*/	
$(".cart-plus-minus").append('<button class="inc qtybutton" disabled><i class="mo-caret-up"></i></button>');
$(".cart-plus-minus").append('<button class="dec qtybutton" disabled><i class="mo-caret-down"></i></button>');
...
$(".qtybtn").on("click", function() {
   	...
   	var $button = $(this);
	var oldValue = $button.parent().find("input").val(); 
    var prodName = $(this).closest("tr").find("input[type='hidden']").val(); // get the related product name 
   
    if ($button.hasClass('inc')) {
	  var newVal = parseFloat(oldValue) + 1;
      $("#cart-load").load( "./inc/cart_process.php?cart_action=inc&product_name="+prodName);
	  ...
	  if (oldValue > 0 && oldValue != 1) {
		var newVal = parseFloat(oldValue) - 1;
        $("#cart-load").load( "./inc/cart_process.php?cart_action=dec&product_name="+prodName);
	...
	$button.parent().find("input").val(newVal);
    $(this).closest("tr").find("span.cart-total-price").load( "./inc/cart_process.php?chngTotalPrice=1&product_name="+prodName);
      $("#order-pro-table").load( "./inc/cart_process.php?checkout_cart=1");
      $(".subTotal").load( "./inc/cart_process.php?chngSubTotalPrice=1");    
      $(".grandTotal").load( "./inc/cart_process.php?chngGrandTotalPrice=1");    
